...
air2_test <- function(x){
  n_y4 <- dim(Y4)[1] - k
  train_results <- array(NA, dim = c(n_y4))  # Initialized to NA
  train_labels <- array(Y4[1:n_y4+k,1], dim = c(n_y4))
  for(i in 1:n_y4){
    TW_i <- Y4[i,1] + dw_dt(Y4[i,1], 4, Y4[i,3], x, i)*k
    train_results[i] <- TW_i
  }
  valid_idx <- !is.na(train_labels) & !is.na(train_results)  # Exclude NA
  return(list(raw = train_labels[valid_idx], prediction = train_results[valid_idx]))
}
