...
path <- "E:/Rdatafile/prediction of lake temp/ESWA-SOURCE-CODE/XGB-13variables-10"
setwd(path)

library(xgboost)
library(mcga)

ori_data <- read.csv(paste0(path,"/Blel_2008_2009_2010_2011.csv"))
...
for(k in 1:12){
  load(paste0(path,"/XGB",mark[k],".Rdata"))
  ...
  sub_results_train <- data.frame(measurements =  label_train,
                                  predictions = pre_train)
  sub_results_val <- data.frame(measurements =  label_val,
                                  predictions = pre_val)
  sub_results_test <- data.frame(measurements =  label_test,
                                  predictions = pre_test)
  write.csv(sub_results_train, paste0(path,"/Results/Training/XGB",mark[k],".csv"))
  write.csv(sub_results_val, paste0(path,"/Results/Validation/XGB",mark[k],".csv"))
  write.csv(sub_results_test, paste0(path,"/Results/Testing/XGB",mark[k],".csv"))
}

write.csv(indicator,paste0(path,"/Results/Indicator_xgb.csv"))
