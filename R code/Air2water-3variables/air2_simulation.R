...
fitness_air2 <- function(x){
  ...
  #Y1
  for(i in 1:n_y1){
    TW_i <- Y1[i,1] + dw_dt(Y1[i,1], Y1[i,2], Y1[i,3], x, i)
    if(k!=1){
      for(j in 1:(k-1)){
        TW_i <- TW_i + dw_dt(TW_i, Y1[i,2], Y1[i,3], x, i+j)
      }
    }
  ...
  #Y2
  for(i in 1:n_y2){
    TW_i <- Y2[i,1] + dw_dt(Y2[i,1], Y2[i,2], Y2[i,3], x, i)
    if(k!=1){
      for(j in 1:(k-1)){
        TW_i <- TW_i + dw_dt(TW_i, Y2[i,2], Y2[i,3], x, i+j)
      }
    }
  ...
  #Y3
  for(i in 1:n_y3){
    TW_i <- Y3[i,1] + dw_dt(Y3[i,1], Y3[i,2], Y3[i,3], x, i)
    if(k!=1){
      for(j in 1:(k-1)){
        TW_i <- TW_i + dw_dt(TW_i, Y3[i,2], Y3[i,3], x, i+j)
      }
    }
...
air2_test <- function(x){
  ...
  #Y4
  for(i in 1:n_y4){
    TW_i <- Y4[i,1] + dw_dt(Y4[i,1], Y4[i,2], Y4[i,3], x, i)
    if(k!=1){
      for(j in 1:(k-1)){
        TW_i <- TW_i + dw_dt(TW_i, Y4[i,2], Y4[i,3], x, i+j)
      }
    }
    train_results[i] <- TW_i
  }
